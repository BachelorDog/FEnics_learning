...
#         Theta_m1=Q.dot(W)
        hp.linalg.MvDSmatMult(Q,W,Theta_m1)
        # truncate eigenvalues for updated LIS
        idx_s=Xi_m1>=threshold_s
        self.Theta_m=hp.linalg.MultiVector(self.v,sum(idx_s))
        ...
        self.Theta_r=hp.linalg.MultiVector(self.v,sum(idx_g))
...
#         rk1_v=np.vstack([self.emp_mean,self.v]).dot(self.Theta_r)
        rk1_v=np.array([self.Theta_r.dot_v(self.model.prior.M*r) for r in [self.emp_mean,self.v]])
...
#                 print('Ah-Oh--')
        ...
        self.Psi_r=hp.linalg.MultiVector(self.v, self.dim_LIS_g)
        hp.linalg.MvDSmatMult(self.Theta_r,W_r,self.Psi_r)
...
#         Theta_r1r=self.Theta_r.T.dot(Theta_r)
        self.Theta_r1r=np.array([self.Theta_r.dot_v(self.model.prior.M*Theta_r[i]) for i in range(Theta_r.nvec())]).T
...
#             v_ref=np.zeros_like(self.v)
        ...
        if self.proposal is 'LI_prior':
            ...
            Psi_r_v=self.Psi_r.dot_v(self.model.prior.M*self.v); Psi_r_v1=self.Psi_r.dot_v(self.model.prior.M*v)
...
#                             self.model._init_states(src4init='map_solution') # reinitialize solution to MAP
            ...
            if (n+1)%self.n_lag==0 and self.update_LIS_m<self.n_max and self.d_F>=self.threshold_LIS:
                ...
                Theta_r_=hp.linalg.MultiVector(self.Theta_r); Xi_r_=self.Xi_r.copy()
                # Update LIS
                if self.parameters['print_level'] > 0:
                    print('\nUpdating LIS...')
    ...
    def save_samp(self):
        ...
        f.close()
        # save global LIS
#         self.Theta_r.export(self.model.Vh[hp.PARAMETER],os.path.join(self.savepath,'gLIS_'+self.filename+'.pvd'))
        self.Theta_r.export(self.model.Vh[hp.PARAMETER],os.path.join(self.savepath,'gLIS_'+self.filename+'.xdmf'))