...
def main():
    ...
    parser.add_argument('step_sizes', nargs='?', type=float, default=[.01,.02,.02,.2,.1,.1,.1])
    parser.add_argument('step_nums', nargs='?', type=int, default=[1,1,4,1,4,1,4])
    parser.add_argument('algs', nargs='?', type=str, default=('pCN','infMALA','infHMC','infmMALA','infmHMC','drinfmMALA','drinfmHMC'))
...
#     geomfun=lambda parameter,geom_opt: elliptic.get_geom(parameter,geom_opt,k=len(elliptic.misfit.obs))
    geomfun=lambda parameter,geom_opt: elliptic.get_geom(parameter,geom_opt,k=5)
    inf_GMC=geoinfMC(unknown,elliptic,geomfun,args.step_sizes[args.algNO],args.step_nums[args.algNO],args.algs[args.algNO])
