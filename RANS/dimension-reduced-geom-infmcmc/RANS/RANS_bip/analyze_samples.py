...
#                         _,_,_,_,soln_count,args=pickle.load(f,encoding='bytes')
                        ...
                        soln_count,num_samp=np.nan,2000 # temp solution
                        try:
                            f_read=pickle.load(f,encoding='bytes')
...
#                             soln_count,num_samp=np.nan,2000 # temp solution
                        ...
                        _solns.append(np.sum(soln_count))
                        found_a=True
                    except:
#                         pass
                        print('Need manual input.')
#                     if self.algs[a]=='DRinfmMALA': # temp solution
#                         _acpt=np.mean([.93,.92]);_time=176169+182686;found_a=True
            if found_a:
...
#     algs=('pCN','infMALA','infHMC','DRinfmMALA','DRinfmHMC','DILI','aDRinfmMALA','aDRinfmHMC')
    ...
    _=ana_samp(algs=algs,dir_name='analysis-L25W8-nofreshinit-yesparacont-2000-adph',PLOT=False,save_fig=True).analyze()
