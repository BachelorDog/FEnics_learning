...
class RANSModel_base:
    ...
    def all_tau(self, x):
        ...
        u,p,k,e = dl.split(x) 
        h2 = h_u2(self.metric, u, self.reg_norm*self.nu*self.nu)
        tau = [ self.tau(self.nu + self.nu_t(k,e), u, self.metric),
                h2*self._u_norm(u),
                self.tau( self.nu + self.nu_t(k,e)/self.sigma_k, u, self.metric),
                self.tau( self.nu + self.nu_t(k,e)/self.sigma_e, u, self.metric)
               ]

        return tau
    
    def stab(self,x, x_test, sigma, x0, xl, true_derivative=False):
        """
        The G-LS stabilization
        """
        r_s = self.strong_residual(x, sigma, x0, xl)
    ...
    def weak_residual(self, x, x_test, sigma, x0, xl):
        """
        The weak residual
        """
        u,p,k,e = dl.split(x)
        ...
        res_u =  dl.Constant(2.)*(self.nu+self.nu_t(k,e))*dl.inner( self.strain(u), self.strain(u_test))*dl.dx \
               + dl.inner(dl.grad(u)*u, u_test)*dl.dx \
               - p*dl.div(u_test)*dl.dx \
               + self.Cd*(self.nu+self.nu_t(k,e))*hbinv*dl.dot(u - self.u_ff, self.tg)*dl.dot(u_test, self.tg)*self.ds_ff \
               - dl.dot( self.sigma_n(self.nu+self.nu_t(k,e), u), self.tg) * dl.dot(u_test, self.tg)*self.ds_ff \
               - dl.dot( self.sigma_n(self.nu+self.nu_t(k,e), u_test), self.tg ) * dl.dot(u - self.u_ff, self.tg)*self.ds_ff
        ...
        res_k = dl.dot(u, dl.grad(k))*k_test*dl.dx \
                + (self.nu + self.nu_t(k,e)/self.sigma_k)*dl.inner( dl.grad(k), dl.grad(k_test))*dl.dx \
                + e*k_test*dl.dx \
                - self.nu_t(kl,el)*self.production(ul)*k_test*dl.dx \
                - self._chi_inflow(ul)*(self.nu + self.nu_t(k,e)/self.sigma_k)*dl.dot(dl.grad(k), self.n)*k_test*self.ds_ff \
                - self._chi_inflow(ul)*(self.nu + self.nu_t(k,e)/self.sigma_k)*dl.dot(dl.grad(k_test), self.n)*(k-self.k_ff)*self.ds_ff \
                + self._chi_inflow(ul)*self.Cd*(self.nu + self.nu_t(k,e)/self.sigma_k)*hbinv*(k-self.k_ff)*k_test*self.ds_ff
                
        res_e = dl.dot(u, dl.grad(e))*e_test*dl.dx \
                + (self.nu + self.nu_t(k,e)/self.sigma_e)*dl.inner( dl.grad(e), dl.grad(e_test))*dl.dx \
                + self.C_e2*e*self._theta(k,e)*e_test*dl.dx \
                - self.C_e1*self.C_mu*kl*self.production(ul)*e_test*dl.dx \
                - self._chi_inflow(ul)*(self.nu + self.nu_t(k,e)/self.sigma_e)*dl.dot(dl.grad(e), self.n)*k_test*self.ds_ff \
                - self._chi_inflow(ul)*(self.nu + self.nu_t(k,e)/self.sigma_e)*dl.dot(dl.grad(e_test), self.n)*(e-self.e_ff)*self.ds_ff \
                + self._chi_inflow(ul)*self.Cd*(self.nu + self.nu_t(k,e)/self.sigma_e)*hbinv*(e-self.e_ff)*e_test*self.ds_ff
    ...
    def strong_residual(self, x, sigma, x0, xl):
        """
        The strong residual
        """
        u,p,k,e = dl.split(x)
        ...
        res_u = -dl.div( dl.Constant(2.)*(self.nu+self.nu_t(kl,el))*self.strain(u) ) + dl.grad(u)*ul + dl.grad(p)
        res_p = dl.div( u )
        res_k = + dl.dot(ul, dl.grad(k)) \
                - dl.div( (self.nu + self.nu_t(kl,el)/self.sigma_k)* dl.grad(k) ) \
                + el \
                - self.nu_t(kl,el)*self.production(ul)
                
        res_e = + dl.dot(ul, dl.grad(e)) \
                - dl.div( (self.nu + self.nu_t(kl,el)/self.sigma_e)* dl.grad(e) ) \
                + self.C_e2*e*self._theta(kl,e) \
                - self.C_e1*self.C_mu*kl*self.production(ul)
                
        #return [sigma*(u-u0) + res_u, res_p, sigma*(k-k0) + res_k, sigma*(e-e0) + res_e]
        return [res_u, res_p, res_k, res_e]
    
                    
    def residual(self, x, x_test, sigma, x0, xl, true_derivative=False):
        """
        ...
        return self.weak_residual(x, x_test, sigma, x0, xl) + self.stab(x, x_test, sigma, x0, xl, true_derivative)
        
    def Jacobian(self, x, x_test, x_trial, sigma, x0, xl, true_derivative = False):
        """
        ...
        if xl is None:
            xl = vector2Function(x.vector(), self.Vh)
        r1 = self.weak_residual(x, x_test, sigma, x0, xl)
        r2 = self.stab(x, x_test, sigma, x0, xl, true_derivative)
        if true_derivative:
            return dl.derivative(r1,x,x_trial) + dl.derivative(r1, xl, x_trial) \
                 + dl.derivative(r2,x,x_trial) + dl.derivative(r2,xl,x_trial)
        else:
            return dl.derivative(r1,x,x_trial) + dl.derivative(r2,x,x_trial)
