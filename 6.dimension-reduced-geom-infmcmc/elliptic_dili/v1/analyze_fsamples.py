...
#             sumry_ESS=np.array(np.loadtxt(ESS_fname,dtype={'names':('method','ESS_min','ESS_med','ESS_max'),'formats':('|S10',np.float,np.float,np.float)},delimiter=','))[None,]
            sumry_ESS=np.array(np.genfromtxt(ESS_fname,dtype=np.dtype([('method','U10'),('ESS_min','<f8'),('ESS_med','<f8'),('ESS_max','<f8')]),delimiter=','),ndmin=1)
        
        for a in range(self.num_algs):
            # samples' ESS
            _acpt=[];_time=[];_ESS=[];_ESS_l=[];_solns=[]
            ...
            for f_i in self.fnames:
                if '_'+self.algs[a]+'_' in f_i:
                    try:
                        ...
                        if self.algs[a] not in ['infMALA','infHMC']:
                            soln_count=soln_count[1] # added temporarily for wrong format in recording
                        f.close()
                        _,_ESS_l_i=self.cal_ESS(loglik[:,np.newaxis])
                        _acpt.append(acpt);_time.append(time)
