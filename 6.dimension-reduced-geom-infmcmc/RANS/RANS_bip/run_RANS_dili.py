...
import sys
sys.path.append( "../../" )
from sampler.DILI_hippy import DILI
...
def main():
    ...
    parser.add_argument('step_sizes', nargs='?', type=float, default=[.15,.25])
...
#     parameter=rans.whtprior.u2v(parameter.vector())
    ...
    dili=DILI(parameter,rans,args.step_sizes,args.props[args.propNO],adpt_stepsz, \
              target_acpt=0.7,fresh_init=fresh_init,para_cont=para_cont,src4init='solution',n_lag=100)
    dili.setup(args.num_samp,args.num_burnin,1,mpi_comm=rans.mpi_comm)
    if rans.rank != 0:
        dili.parameters['print_level']=-1
