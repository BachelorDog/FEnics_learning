...
class RANS:
    def __init__(self,nozz_w=1.,nx=50,ny=100):
        """
        ...
        self.D=nozz_w
        # domain size
#         self.box=[5.*self.D, 35.*self.D, 0., 10.0*self.D]
        self.box=[5.*self.D, 30.*self.D, 0., 8.0*self.D]
        # mesh size
        self.nx=nx; self.ny=ny;
        # geometry
        self.geo=FreeJet_Geometry(self.box, self.nx, self.ny)
        self.mpi_comm = self.geo.mesh.mpi_comm()
        self.rank = dl.MPI.rank(self.mpi_comm)
        self.nproc = dl.MPI.size(self.mpi_comm)
#         if self.nproc > 1:
#             Random.split(self.rank, self.nproc, 1000000, 1)
#         # set FEM
#         self._set_FEM()
        
        # constants of the model
        self.nu = dl.Constant(1e-4)
        self.C_mu = dl.Constant(0.09)
        self.sigma_k = dl.Constant(1.00)
        self.sigma_e = dl.Constant(1.30)
        self.C_e1 = dl.Constant(1.44)
        self.C_e2 = dl.Constant(1.92)
        
#         # set prior
#         self._set_prior()
#         # set forms and misfit
#         self._set_pde(); self._set_misfit();
    ...
    def _set_FEM(self):
        ...
        u_bc_axis = dl.DirichletBC(self.Vh_STATE.sub(0).sub(1), self._scalar_zero, self.geo.boundary_parts, self.geo.AXIS)
        ...
        u_bc_axis0 = dl.DirichletBC(self.Vh_STATE.sub(0).sub(1), self._scalar_zero, self.geo.boundary_parts, self.geo.AXIS)
    ...
    def _set_pde(self,**kwargs):
        ...
        self.pde = RANSProblem(self.Vh, model_phys, self.ess_bcs, self.ess_bcs0, self.states_fwd.vector(), verbose=(self.rank == 0) )
...
#         # 7. Set up the inverse problem model, which consits of a pde, a prior, and a misfit
    ...
    def _soln_adj(self,parameter=None):
        ...
        x = [self.states_fwd.vector(),parameter,self.model_stat.generate_vector(ADJOINT)]
        
        self.model_stat.solveAdj(x[ADJOINT], x, 1e-9)
...
#         self.states_adj.vector().axpy(1.,x[ADJOINT])
    ...
    def _get_grad(self,parameter=None,MF_only=True):
        ...
        grad = self.model_stat.generate_vector(PARAMETER)
#         if MF_only:
#             # misfit gradient
#             self.pde.eval_da(x,grad)
#         else:
#             # negative log-posterior gradient
#             gradnorm = self.model_stat.evalGradientParameter(x, grad)
        gradnorm = self.model_stat.evalGradientParameter(x, grad, misfit_only=MF_only)
        return grad
    ...
    def _get_HessApply(self,parameter=None,GN_appx=True,MF_only=True):
        ...
        self.model_stat.setPointForHessianEvaluations(x)
        HessApply = ReducedHessian(self.model_stat, 1e-9, gauss_newton_approx=GN_appx, misfit_only=MF_only)
        return HessApply

    def get_geom(self,parameter=None,geom_ord=[0],whitened=False,log_level=dl.ERROR,**kwargs):
        """
        ...
        if any(s>=0 for s in geom_ord):
            loglik = -self._get_misfit(parameter,fresh_init=kwargs.pop('fresh_init',False),src4init=kwargs.pop('src4init','map_solution'),para_cont=kwargs.pop('para_cont',True))
        ...
        if any(s>=1.5 for s in geom_ord):
            HessApply = self._get_HessApply(parameter,kwargs.pop('GN_appx',True),kwargs.pop('MF_only',True)) # Hmisfit if MF is true
            if whitened:
                HessApply = wht_Hessian(self.whtprior,HessApply)
            if np.max(geom_ord)<=1.5:
                # adjust the gradient
                Hu = self.model_stat.generate_vector(PARAMETER)
                HessApply.mult(parameter,Hu)
                agrad.axpy(1.,Hu)
                if not kwargs.pop('MF_only',True):
                    Ru = self.model_stat.generate_vector(PARAMETER)
...
#                 print('Double Pass Algorithm. Requested eigenvectors: {0}; Oversampling {1}.'.format(k,p))
    ...
    def get_eigs(self,parameter=None,whitened=False,**kwargs):
        ...
        # solve the forward problem
        self._soln_fwd(parameter,fresh_init=kwargs.pop('fresh_init',False),src4init=kwargs.pop('src4init','map_solution'),para_cont=kwargs.pop('para_cont',True))
        # solve the adjoint problem
        self._soln_adj(parameter)
        # get Hessian Apply
        HessApply = self._get_HessApply(parameter,kwargs.pop('GN_appx',True),kwargs.pop('MF_only',True)) # Hmisfit if MF is true
...
#             print('Double Pass Algorithm. Requested eigenvectors: {0}; Oversampling {1}.'.format(k,p))
    ...
    def get_MAP(self,rand_init=False,SAVE=True):
        ...
        solver = ReducedSpaceNewtonCG(self.model_stat)
        ...
        if SAVE:
            ...
            MAP_file.write(vector2Function(self.misfit.d, self.Vh[STATE]), "data")
            MAP_file.write(vector2Function(MAP[STATE]-self.misfit.d, self.Vh[STATE]), "misfit")
    ...
    def test(self,SAVE=False,PLOT=False,chk_fd=False,h=1e-4):
        ...
        self.prior.init_vector(noise,"noise")
#         Random.normal(noise, 1., True)
        parameter = self.model_stat.generate_vector(PARAMETER)
#         self.prior.sample(noise,parameter)
        ...
        start = time.time()
        loglik,grad,_,_ = self.get_geom(parameter,geom_ord=[0,1],whitened=False,src4init='solution',fresh_init=False)
        ...
        if chk_fd:
            ...
            # random direction
            Random.normal(noise, 1., True)
            v = self.model_stat.generate_vector(PARAMETER)
...
#             rdiff_gradv = np.abs(dloglikv_fd-dloglikv)/np.linalg.norm(v)
            ...
            # random direction
            Random.normal(noise, 1., True)
            w = self.model_stat.generate_vector(PARAMETER)
