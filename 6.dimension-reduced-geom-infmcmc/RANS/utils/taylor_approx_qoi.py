...
class TaylorApproximationQOI:
    ...
    def __init__(self, rqoi, distribution):
        ...
        Constructor
        INPUT:
        - rqoi: an object of type ReduceQOI that describes the parameter-to-qoi map
    ...
    def computeLowRankFactorization(self, innerTol=1-9, k = 50):
        """
        ...
        INPUTS:
        - innerTol: the tolerance used to solve the forward, adj, and incremental problems for the computation of
                    the state, adj, incremental state, incremental adj variables for the Hessian application.
        - k: the number of eigenpairs to be retained in the LowRank factorization of the prior-preconditioned Hessian.
        """
        
        self.rqoi.solveFwd(self.x_bar[STATE], self.x_bar, innerTol)
        self.rqoi.solveAdj(self.x_bar[ADJOINT], self.x_bar, innerTol)
        
        self.q_bar = self.rqoi.eval(self.x_bar)
        self.rqoi.evalGradientParameter(self.x_bar, self.g_bar)
        
        self.H = self.rqoi.reduced_hessian(x=self.x_bar, innerTol=innerTol)
        Omega = MultiVector(self.x_bar[PARAMETER], k)
        for i in xrange(k):
            Random.normal(Omega[i], 1., True)
        if hasattr(self.distribution, "R"):
            self.d, self.U = doublePassG(self.H, self.distribution.R, self.distribution.Rsolver, Omega, k)
        else:
            self.d, self.U = doublePassG(self.H, self.distribution.Hlr, self.distribution.Hlr, Omega, k)
