...
Modified on Jan 12, 2017 by slan
Modified on Feb 8, 2017 by slan
Modified on March 31, 2017 by slan
Modified on April 2, 2017 by slan
Modified on April 10, 2017 by slan
Modified on April 29, 2017 by slan
Modified on May 3, 2017 by slan
Modified on July 2, 2017 by slan
'''
...
class RANSProblem(PDEProblem):
    ...
    def _solveFwdParamCont(self, state, x,tol):
        ...
        while 1.:
            ...
            if solver.converged == False and dalpha == 1e-4:
                ...
                alpha += dalpha
                convg = self._solveFwdTransientCont(new_state,x, tol)
                # break the long loop (1e4) for slowly increasing alpha by dalpha
                if convg:
                    state.zero()
                    state.axpy(1., new_state) # take current state as solution
                    nTry_TransientCont = 0 # reset the counter
                    break
                else:
                    nTry_TransientCont += 1
                    if nTry_TransientCont >= 10:
                        nTry_TransientCont = 0
                        if self.verbose:
                            print "10 consecutive failures on ParamCont! Raise error..."
                        raise RuntimeError('Trapped in bad solution!')
    ...
    def _solveFwdTransientCont(self,state,x, tol):   
        ...
        while norm_steady_state_r > tol:
            ...
            # raise RuntimeError if the solver is trapped in a dead loop
            if solver.final_it == 0 and not solver.converged:
                raise RuntimeError('Trapped in bad solution!')
        ...
        return norm_steady_state_r <= tol
